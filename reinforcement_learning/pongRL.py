...
class PongEnvironment:
    ...
    def dibujar_frame(self):
        ...
        if self.lives <=0:
            ...
            plt.pause(5)
        elif self.total_reward >= MAX_REWARDS:
...
def play(rounds=5000, max_life=3, discount_factor = 0.1, learning_rate = 0.1,
    ...
    for played_games in range(0, rounds):
        ...
        itera=0
        while (done != True) and (itera < 3000 and game.total_reward < MAX_REWARDS):
            old_state = np.array(state)
            next_action = learner.get_next_step(state, game)
            state, reward, done = game.step(next_action, animate=animate)
            if game.total_reward >= MAX_REWARDS and animate:
                game.dibujar_frame()
            if rounds > 1:
...
learner, game = play(rounds=ROUNDS, discount_factor = 0.2, learning_rate = 0.1, ratio_explotacion=0.85)
